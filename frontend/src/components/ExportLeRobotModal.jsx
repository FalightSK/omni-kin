...
export default function ExportLeRobotModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-sm animate-fade-in">
      <div className="relative w-full max-w-lg bg-[#0a0a0a] border border-neutral-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        {/* Modal Header */}
        ...
        <div className="p-5 flex flex-col gap-4 text-left">
          {exportResult ? (
            ...
            <div className="flex flex-col gap-3 py-2">
              ...
              <div className="bg-[#050505] p-3.5 rounded-xl border border-neutral-800 text-xs font-mono space-y-1.5 text-neutral-300">
                <div><span className="text-neutral-500">Path:</span> {exportResult.export_path}</div>
                <div><span className="text-neutral-500">Embodiment:</span> {exportResult.robot_type.toUpperCase()}</div>
                <div><span className="text-neutral-500">Mode:</span> {exportResult.trajectory_mode}</div>
                <div><span className="text-neutral-500">Auto-Trim:</span> {exportResult.auto_trim ? 'Enabled (Feasible Only)' : 'Disabled'}</div>
                <div><span className="text-neutral-500">Total Episodes:</span> {exportResult.total_episodes}</div>
                <div><span className="text-neutral-500">Total Frames:</span> {exportResult.total_frames}</div>
              </div>
              <p className="text-[11px] text-neutral-400 leading-relaxed">
                The dataset is ready for training policies with <code className="bg-neutral-900 border border-neutral-800 px-1 py-0.5 rounded text-neutral-300">lerobot-train</code> or inspecting via LeRobot Visualizer.
              </p>
